...
def main():
    ...
    path_fact = f"s3://{bucket_name}/analytics/fact_orders/"
    try:
    ...
    # Validação da presença das colunas de partição (order_year, order_month)
    info("Checando presença das colunas de partição...")
    if "order_year" in df_fact.columns and "order_month" in df_fact.columns:
        print(f"[OK] Colunas de partição 'order_year' e 'order_month' encontradas na fato. Anos distintos: {df_fact['order_year'].unique().tolist()}")
    else:
        error("Colunas de partição 'order_year' e/ou 'order_month' NÃO encontradas na fato.")
