...
def resolve_sql_path() -> str:
    ...
    # Busca relativa subindo até a raiz do repositório
    candidates.append(
        os.path.normpath(
...
def load_connection_config(filename: str = "rds_connection.env") -> dict:
    base_dir = os.path.dirname(os.path.abspath(__file__))
    # Busca rds_connection.env na pasta pai primeiro
    filepath = os.path.join(os.path.dirname(base_dir), filename)
    if not os.path.isfile(filepath):
        filepath = os.path.join(base_dir, filename)
    env = {}
    if os.path.isfile(filepath):
        info(f"Carregando configurações de: {filepath}")
        ...
        warn(f"Arquivo {filepath} não encontrado.")
    return env
...
def load_data():
    ...
    try:
        with connection.cursor() as cursor:
            # Desativa verificações de FK para evitar erros de drop/create fora de ordem
